import JeaArticles from "../../../Articles/JeaArticles/JeaArticles";


const JeaScope = () => {
    return (
        <div className="w-full">
            <h2 className="text-xl font-bold text-[#91000D]">Aims & Scope</h2>

            <div className="flex items-center py-8 gap-24">
                Aeronautics and Astronautics,
                Chemical Engineering,
                Nanoscience and Molecular Engineering,
                Civil Engineering,
                Environmental Engineering,
                Computer Science and Engineering,
                Electrical Engineering,
                Engineering,
                Human Centered Design and Engineering,
                Human Centered Design and Engineering,
                Technical Communication,
                Industrial Engineering,
                Materials Science and Engineering,
                Mechanical Engineering,
                Mechanical Engineering Industrial Engineering,
                Bioresource and Science Engineering,
                Environmental Science and Resource Management,
                Marine Engineering,
                Design Drawing,
                World Architecture,
                Architectural Design,
                Design Studio,
                Architectural Design Drawing,
                Architectural Drawing,
                Structures,
                Energy and Environmental Systems,
                Construction Materials and,
                Architecture of the Ancient World,
                Architectural history,
                Romanesque, Gothic, and Renaissance Architecture,
                History of Modern Architecture,
                Architectural Theory,
                Computers in Architecture,
                Architectural Problems,
                Integrated Design Build Studio,
                Architectural Photography,
                Architectural Sketching,
                Watercolor Drawing,
                Structural Unit Masonry,
                Materials and Processes,
                Environmental Control Principles,
                Color and Light,
                Environmental Lighting,
                Building Acoustics,
                Visions of the Japanese House,
                Africa and Middle East Seminar,
                South Asian Architecture,
                Modern Architecture and the Decorative Arts,
                Nineteenth-Century Architecture,
                Spatial Composition in Architecture,
                Theories of Representation,
                Gender and Architecture,
                Residential Architectural Practice,
                Design and the International Building Code,
                CAD and Working Drawings,
                Modeling and Rendering,
                Web Weaving,
                Design of Virtual Environments,
                Computer Graphics Programming for Design,
                American Architecture,
                Simulation Based Design,
                Architectural Design Studio,
                Architectural Design Studio Options,
                Integrated Building Systems,
                Active Control Systems for Building Operation,
                Construction Materials and Assemblies,
                Green Technology,
                Traditional Building Methods,
                The Arts & Crafts Movement and Its Legacies,
                Urban Design Theory,
                Environmental Design and Well-Being,
                Design and Construction Law,
                Ethical Practice,
                Contemporary Architectural Practice,
                Technical Issues in Preservation Design,
                Computational Lighting Design,
                Theory of Design Computing,
                Urban and Preservation Issues in Design,
                Architecture and Landscape.
            </div>

            <div className='py-2'>
                <h2 className='font-medium text-[#91000D] py-2'>Current Issue:</h2>
                <div>
                    <JeaArticles></JeaArticles>
                </div>
            </div>
        </div>
    );
};

export default JeaScope;