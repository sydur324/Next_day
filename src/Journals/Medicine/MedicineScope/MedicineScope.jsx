import MedicineArticles from "../../../Articles/MedicineArticles/MedicineArticles";

const MedicineScope = () => {
    return (
        <div className="w-full">
            <h2 className="font-medium text-[#91000D]">Aims & Scope</h2>

            <div className="flex items-center py-4 gap-24">
                <p className="text-[15px]">
                    Organization of the body,
                    Physiology and pharmacology,
                    Biochemistry and medical genetics,
                    Medical sociology,
                    Systems of the body: Integrative aspects,
                    The nervous system,
                    Principles of pathology,
                    Psychology for medicine,
                    Principles of clinical anatomy,
                    Histology,
                    Embryology,
                    First Aid,
                    Microbiology,
                    Medical Immunology,
                    Basis for Patient Care,
                    Psychology,
                    Pathological Anatomy,
                    Pathological Physiology,
                    Pharmacology,
                    Physical Examination in Internal Medicine,
                    Internal Medicine,
                    Medical Psychology and Communication with the Patient,
                    Pathological Physiology,
                    Surgery,
                    Pharmacology,
                    Dentistry,
                    Neurology,
                    Dermatovenereology,
                    Diagnostic Radiology and Nuclear Medicine,
                    Sports Medicine,
                    Psychiatry,
                    Differential Diagnosis in Internal Medicine,
                    General Medicine,
                    Rehabilitation Medicine,
                    Ophthalmology,
                    Otorhinolaryngology,
                    Forensic Medicine,
                    Oncology and Radiotherapy,
                    Gynecology and Obstetrics,
                    Gynecology and Obstetrics,
                    Pediatrics,
                    Pediatrics,
                    Hygiene and Preventive Medicine,
                    Pathobiochemistry and Disorders of Metabolism and Nutrition,
                    Basis of Harvesting and Preservation of Tissues for Transplantation,
                    Myoskeletal Medicine,
                    Clinical Sexology,
                    Emergency and Critical Care,
                    Clinical Pharmacology,
                    Psychotherapy,
                    Transfusion medicine,
                    Clinical Anatomy,
                    Stem Cells and Regenerative Medicine,
                    Medical Information Resources on Internet,
                    Cognitive-Behavioral Therapy,
                    Neurosciences,
                    Family Systems Medicine,
                    Microbiological Aspects of Tropical Infectious Diseases,
                    Toxicology,
                    Biology of Tumor Cells,
                    New Trends in Pharmacology,
                    Modern Research Methods in Pharmacology,
                    Cellular Pathobiochemistry,
                    Degenerative diseases of the spine,
                    Medical Informatics,
                    Medicinal Chemistry,
                    Pharmaceutics,
                    Pharmacy,,
                    Pharmacy Practice,
                    Pharmacy Regulatory Affairs,
                    Infectious Diseases and Epidemiology,
                    Infectious Diseases and Epidemiology,
                </p>
            </div>

            <div className='py-2'>
                <h2 className='font-medium text-[#91000D] py-2'>Current Issue</h2>
                <div>
                    <MedicineArticles></MedicineArticles>
                </div>
            </div>
        </div>
    );
};

export default MedicineScope;